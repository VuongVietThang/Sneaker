...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kiểm tra tên
    if (empty($name)) {
        $errors['name'] = 'Size không được để trống.';
    }

    $sizeModel = new Size();
    if ($sizeModel->isSizeExists($name)) {
        $errors['name'] = 'Size đã tồn tại.';
    }

    // Nếu không có lỗi, tiến hành thêm size
    if (!array_filter($errors)) { // Kiểm tra xem có lỗi nào không
        if ($sizeModel->addSize($name)) {
...
<body>
    <div class="container">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <button class="btn btn-success mb-3" style="margin-bottom: 15px" type="submit" name="submit">ADD SIZE</button>
